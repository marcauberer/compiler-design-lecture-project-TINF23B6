...
public class TypeChecker extends ASTSemaVisitor<ExprResult> {
    ...
    public ExprResult visitSwitchCaseStmt(ASTSwitchCaseStmtNode node) {
        // Visit the condition to evaluate its type
        ExprResult conditionResult = visit(node.getCondition());
        List<ASTCaseStmtNode> caseBlocks = node.getCaseBlocks();

        // Visit all case blocks to evaluate their types
        for (ASTCaseStmtNode caseBlock : caseBlocks) {
            // Visit the case literal to get its type
            ASTLiteralNode literal = caseBlock.getLiteral();
            if (literal != null) {
                ExprResult caseResult = visit(literal);
                if (!caseResult.getType().is(conditionResult.getType().getSuperType())) {
                    throw new SemaError(literal, "Case value must be of type " + conditionResult.getType().getSuperType());
                }
            }

            // Visit the case block's statements
            visit(caseBlock.getStmtLst());
        }

        // Visit default block if it exists
        ASTDefaultStmtNode defaultBlock = node.getDefaultBlock();
        if (defaultBlock != null) {
            visit(defaultBlock);
        }

        // The switch statement itself evaluates to void (TYPE_INVALID)
